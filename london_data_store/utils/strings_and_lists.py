...
class StringOperations:
    ...
    def _convert_string_to_numbers(self, d_type: str, notations=None) -> int | float:
        ...
        try:
            ...
            raise ConversionError(f"'{number}' could not be converted to float") from e
...
# Backward-compatible alias for the typo
ClassInitiationError = ClassIntiationError
...
class ListOperations:
    ...
    def search_string(self):
        ...
        if self._search_string is None or self._search_string == "":
            raise ClassIntiationError("Initiate class by assigning 'search_string' as kwargs")
    ...
    def _get_matching_scores_for_string(self):
        ...
        Returns:
            A list of floats representing similarity ratios (0 to 1) for each non-empty
            string in search_list compared to search_string.
        """
    ...
    def get_best_matching_string(self) -> str:
        ...
        Returns:
            str: The string from the list with the highest similarity ratio to search_string.
        """
    ...
    def search_list_by_snowball(self):
        ...
        Returns:
            Matching strings from search_list, or None if no matches found.
        """
        stemmer = _get_stemmer()
        filtered = [x for x in self.search_list if x and stemmer.stem(self.search_string) in stemmer.stem(x)]
        if filtered:
            return filtered
        else:
            return None
    ...
    def search_list_by_string_for_metric(self, search_metric: float | str):
        ...
        Args:
            search_metric (float | str): Filtering metric. Float threshold,
                or "mean", "median", "0.25", "0.75", "0.5".
        ...
        if search_metric in str_metrics:
            if search_metric == "mean":
                threshold = statistics.mean(matching_scores)
            elif search_metric == "median":
                threshold = statistics.median(matching_scores)
            elif search_metric == "mode":
                threshold = max(set(matching_scores), key=matching_scores.count)
    ...
    def get_unique_sorted_elements_by_key(self, id_key: str, sort_key: str = None) -> list:
        ...
        Args:
            ...
            sort_key (str, optional): The key used to sort the unique elements. Defaults to None.
        ...
        if self.search_list:
            for key in [id_key, sort_key]:
                if key and key not in self.search_list[0]:
                    raise KeyError(f"'{key}' not found. Available keys: {', '.join(self.search_list[0])}")
            ...
            if sort_key:
                unique_list.sort(key=lambda x: x.get(sort_key))
            return unique_list
